...
const DetailSekolahKristen = () => {
  ...
  const getSekolahById = async (idSekolah) => {
    try {
      const response = await axios.get(
        `http://localhost:5000/sekolah-kristen/${idSekolah}`
      );
   ...
   const hapusDokumen = async (id_dok) => {
    try {
      await axios.delete(`http://localhost:5000/dokumen-sekolah-kristen/${id_dok}`);
      getDokumen(id);
    } catch (error) {
      console.log(error);
    }
  ...
  const getDokumen = async (id) => {
    try {
      const response = await axios.get(
        `http://localhost:5000/dokumen-sekolah-kristen/data/${id}`
      );
