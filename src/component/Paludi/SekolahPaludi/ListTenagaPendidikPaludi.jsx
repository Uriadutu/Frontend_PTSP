...
import AddTenagaKependidikanPaludiModal from "../../Modal/PaludiModal/SekolahKristenModal/AddTenagaKependidikanPaludiModal";
import EditTenagaKependidikanPaludiModal from "../../Modal/PaludiModal/EditTenagaKependidikanPaludiModal";

const ListTenagaPendidikPaludi = () => {
  ...
  const getGuruBySekolah = async (id) => {
    try {
      const response = await axios.get(
        `http://localhost:5000/tenagakristen/sekolah/${id}`
      );
  ...
  const hapusGuru = async (idTenaga) => {
    try {
      await axios.delete(`http://localhost:5000/tenagakristen/${idTenaga}`);
      getGuruBySekolah(idsekolah)
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <div className="contain">
      {openModalAdd && (
        <AddTenagaKependidikanPaludiModal
              ...
              >
                ...
                <td className="py-3 px-6 text-center flex justify-around whitespace-nowrap">
                  <button
                    onClick={() =>
                      navigate(
                        `/paludi/data-tenaga-kependidikan/detail-tenaga-kependidikan/${item && item.id}`
                      )
                    }
                    className="detail"
                    title="Lihat"
...
export default ListTenagaPendidikPaludi;
