...
const Masjid = () => {
  ...
  const getRumahIbadahIslam = async () => {
    try {
      const response = await axios.get('http://localhost:5000/rumah-ibadah-islam');
    ...
    <div className="contain">
      ...
      <div className="flex justify-between mb-4">
        <div className="flex items-center gap-2">
          <button onClick={() => setOpenModalAdd(true)} className="btn-add hidden sm:block">
            Tambah Masjid
          </button>
          <button onClick={downloadExcel} className="btn-download hidden sm:block">
            Export ke Excel
          </button>
          <button onClick={() => setOpenModalAdd(true)} className="btn-add sm:hidden block">
            <IoAdd color="white" />
          </button>
          <button onClick={downloadExcel} className="btn-download sm:hidden block">
          ...
          <tbody className="text-gray-600 text-sm font-light">
            {filteredDataMasjid.map((item, index) => (
              <tr key={index} className="border-b border-gray-200 hover:bg-gray-100">
                <td className="py-3 px-6 text-left">{(currentPage - 1) * masjidPerPage + index + 1}</td>
